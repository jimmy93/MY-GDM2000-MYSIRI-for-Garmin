...
# finds the bundled/available Java, and compiles the data field + widget to .prg files.
...
$sdk = (Get-Content $cfg | Where-Object { $_.Trim() -ne "" } | Select-Object -First 1).Trim().TrimEnd('\')
Write-Host "SDK: $sdk"
...
# --- Build both targets to .prg files ---
$out = Join-Path $PSScriptRoot "build"
New-Item -ItemType Directory -Force -Path $out | Out-Null

$targets = @(
    @{ Jungle = "monkey_datafield.jungle"; Out = "rso_datafield.prg"; Tag = "datafield" },
    @{ Jungle = "monkey_widget.jungle";   Out = "rso_widget.prg";    Tag = "widget" }
)
...
Write-Host ""
Write-Host "All builds succeeded. Executables generated in $out"